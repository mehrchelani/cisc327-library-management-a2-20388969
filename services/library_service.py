...
def return_book_by_patron(patron_id: str, book_id: int) -> Tuple[bool, str]:
    ...
    if patron_id == "":
        return False, "patron is required"
    ...
    update_availability = update_book_availability(book_id, +1)
...
def calculate_late_fee_for_book(patron_id: str, book_id: int) -> Dict:
    ...
    active_borrow_records = get_patron_borrowed_books(patron_id)
    matching_record = None
    for record in active_borrow_records:
        current_book_id = record.get('book_id')
    ...
    due_date = matching_record['due_date'].date()
    date_today = datetime.now().date()
    days_overdue = (date_today - due_date).days
    if days_overdue <= 0:
        return {
            'fee_amount': 0.00,
            'days_overdue': 0,
    ...
    if days_overdue <= 7:
        ...
        fee_amount = (0.50 * 7) + 1.0 * (days_overdue - 7)
        
    if fee_amount > 15.00:
        fee_amount = 15.00
        
        
    status = f"overdue by {int(days_overdue)} days"
...
def search_books_in_catalog(search_term: str, search_type: str) -> List[Dict]:
    ...
    if s_type == "title":
        q_case_insensitive = q.lower()
        for book in all_books:
            title =str(book.get("title", "")).lower()
            if q_case_insensitive in title:
                results.append(book)
        return results
    ...
    if s_type == "author":
        ...
        for book in all_books:
            ...
            if q_case_insensitive in author:
                results.append(book)
        return results
...
def get_patron_status_report(patron_id: str) -> Dict:
    ...
    if not patron_id or not patron_id.isdigit() or len(patron_id) != 6:
        return {
            ...
            'num_books_currently_borrowed': 0,
            'borrowing_history': [],
            'status': 'Invalid patron ID',
        }
    ...
    for book in records:
        fee = calculate_late_fee_for_book(patron_id, book['book_id'])  
        total_fees += fee.get('fee_amount', 0.0)

    # get currently borrowed list
    for book in records:
    ...
    return {
        ...
        'num_books_currently_borrowed': get_patron_borrow_count(patron_id),
        'borrowing_history': [],
    }
